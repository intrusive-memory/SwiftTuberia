...
| 6 | SHA-256 checksums on descriptors | T4 | ✅ DONE | REQ-T4 (S2 `dc88d6d`) — all 11 `ComponentFile` entries in `CatalogRegistration.swift` now carry `sha256:` and `expectedSizeBytes:`. Count is 11 (5 T5-XXL safetensors shards + 4 T5-XXL metadata files + 2 SDXL VAE files); plan originally assumed 6, but T5 is sharded across 5 safetensors + 4 metadata files. Integrity-verification loop no longer silently no-ops. |
...
| 12 | CDN upload workflow checksum verification | — | ✅ DONE | REQ-CDN-01 (S7 `f4e6939`) — `VerifyComponentManifest` SwiftPM executable (`Tools/VerifyComponentManifest/main.swift`) compares per-file `{path, size, sha256}` from the uploaded `manifest.json` against `CatalogRegistration` descriptors. `ensure-model-cdn.yml` runs the check after `Verify upload`; exits non-zero on any divergence. |
