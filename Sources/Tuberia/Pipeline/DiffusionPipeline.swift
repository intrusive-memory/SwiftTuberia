...
>: GenerationPipeline {
    ...
    private static func validateAssembly(
        ...
        if encoder.outputEmbeddingDim != backbone.expectedConditioningDim {
            throw PipelineError.incompatibleComponents(
                ...
                reason: "Embedding dimension mismatch: encoder produces \(encoder.outputEmbeddingDim) but backbone expects \(backbone.expectedConditioningDim)"
            )
        }
        ...
        if encoder.maxSequenceLength != backbone.expectedMaxSequenceLength {
            throw PipelineError.incompatibleComponents(
                ...
                reason: "Sequence length mismatch: encoder produces \(encoder.maxSequenceLength) but backbone expects \(backbone.expectedMaxSequenceLength)"
            )
        }
        ...
        if backbone.outputLatentChannels != decoder.expectedInputChannels {
            throw PipelineError.incompatibleComponents(
                ...
                reason: "Latent channel mismatch: backbone produces \(backbone.outputLatentChannels) channels but decoder expects \(decoder.expectedInputChannels)"
            )
        }
        ...
        if supportsImageToImage {
            guard decoder is any BidirectionalDecoder else {
                throw PipelineError.incompatibleComponents(
                    ...
                    outlet: "Decoder",
                    reason: "Recipe declares supportsImageToImage but decoder does not conform to BidirectionalDecoder"
                )
            }
        }
    }
    ...
    public func loadModels(progress: @Sendable (Double, String) -> Void) async throws {
        let weightedSegments: [(segment: any WeightedSegment, componentId: String?, role: PipelineRole, quantization: QuantizationConfig)] = [
        ...
        for (stepIndex, timestep) in timesteps.enumerated() {
            ...
            do {
                if useCFG, let uncondEmb = unconditionalOutput {
                    ...
                    let guidedPrediction = uncondPrediction + request.guidanceScale * (condPrediction - uncondPrediction)
