...
struct VAEMidBlockTests {
  ...
  func attentionInsideMidBlockNocrash() {
    ...
    #expect(
      out.shape == [1, 16, 16, 512],
      "VAEMidBlock with 16×16 spatial must not crash and must output [1, 16, 16, 512]")
  }
}
...
struct VAEUpBlockTests {
  ...
  func withUpsampleDoublesSpace() {
    ...
    #expect(
      out.shape == [1, 16, 16, 512],
      "With upsample, spatial dims must double: expected [1, 16, 16, 512], got \(out.shape)")
  }

  @Test("block without upsample preserves spatial dims")
  func withoutUpsamplePreservesSpace() {
    let block = VAEUpBlock(
      inChannels: 256, outChannels: 128, numResnetBlocks: 3, addUpsample: false)
    ...
    #expect(
      out.shape == [1, 8, 8, 128],
      "Without upsample, spatial dims preserved: expected [1, 8, 8, 128], got \(out.shape)")
  }
  ...
  func evalBeforeUpsamplePreventsCrash() {
    ...
    #expect(
      out.shape == [1, 32, 32, 512],
      "Regression: Upsample must not crash with ndim=0 input; expected [1, 32, 32, 512], got \(out.shape)"
    )
  }
}
