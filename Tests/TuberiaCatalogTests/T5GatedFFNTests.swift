...
struct GeluNewAccuracyTests {
  ...
  func geluNewConstantCorrect() {
    ...
    #expect(
      abs(usedInCode - expected) < 1e-7,
      "Constant mismatch: expected \(expected), got \(usedInCode)")
  }
}
...
struct T5RMSNormTests {
  ...
  func normalizesRMS() {
    ...
    #expect(
      abs(rmsLarge - 1.0) < 0.001, "RMS of large input after norm should be ≈1.0, got \(rmsLarge)")
