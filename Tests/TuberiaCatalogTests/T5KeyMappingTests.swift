...
struct T5KeyMappingTests {
    ...
    func allLayersAttentionQKVOMapping() {
        for i in 0..<24 {
            ...
            #expect(T5XXLEncoder.mapKey(qKey) == "blocks.\(i).attention.q",
                    "Layer \(i): Q key mismatch")
            #expect(T5XXLEncoder.mapKey(kKey) == "blocks.\(i).attention.k",
                    "Layer \(i): K key mismatch")
            #expect(T5XXLEncoder.mapKey(vKey) == "blocks.\(i).attention.v",
                    "Layer \(i): V key mismatch")
            #expect(T5XXLEncoder.mapKey(oKey) == "blocks.\(i).attention.o",
    ...
    func allLayersPreAttnNormMapping() {
        for i in 0..<24 {
            ...
            #expect(T5XXLEncoder.mapKey(key) == "blocks.\(i).pre_attn_norm.weight",
    ...
    func allLayersPreFFNNormMapping() {
        for i in 0..<24 {
            ...
            #expect(T5XXLEncoder.mapKey(key) == "blocks.\(i).pre_ffn_norm.weight",
    ...
    func allLayersFFNMapping() {
        for i in 0..<24 {
            ...
            #expect(T5XXLEncoder.mapKey(wi0Key) == "blocks.\(i).ffn.wi_0",
                    "Layer \(i): ffn.wi_0 mismatch")
            #expect(T5XXLEncoder.mapKey(wi1Key) == "blocks.\(i).ffn.wi_1",
                    "Layer \(i): ffn.wi_1 mismatch")
            #expect(T5XXLEncoder.mapKey(woKey) == "blocks.\(i).ffn.wo",
                    "Layer \(i): ffn.wo mismatch")
        }
    }

    @Test("Complete count: 24 layers × 11 keys + embedding + final_norm + relative_position_bias = 267 mapped keys")
    func totalKeyCount() {
        ...
        #expect(mapped.count == expectedMappedCount,
    ...
    func noDuplicateMappedKeys() {
        ...
        #expect(uniqueKeys.count == mappedKeys.count,
