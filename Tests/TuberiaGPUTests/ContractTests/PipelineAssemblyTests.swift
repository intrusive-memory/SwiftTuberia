...
struct PipelineAssemblyTests {
  ...
  func successfulAssembly() throws {
    ...
    #expect(
      pipeline.memoryRequirement.phasedMemoryBytes <= pipeline.memoryRequirement.peakMemoryBytes)
  }
  ...
  func memoryRequirementCalculation() throws {
    ...
    let recipe = StandardMockRecipe(
      ...
      schedulerConfig: .init(),
      backboneConfig: .init(
        conditioningDim: 4096, latentChannels: 4, maxSequenceLength: 120,
        estimatedMemory: backboneMem),
